...
export class PdfXmlEditorProvider implements vscode.CustomReadonlyEditorProvider<PdfDocument> {
  ...
  async resolveCustomEditor(document: PdfDocument, webviewPanel: vscode.WebviewPanel): Promise<void> {
  ...
  private async openEmbeddedXml(pdfUri: vscode.Uri, pdfBytes: Uint8Array): Promise<void> {
    ...
    if (!this.formPanels.has(pdfUri)) {
      await vscode.commands.executeCommand('vscode.setEditorLayout', {
        orientation: 0,
        groups: [{ size: 0.5 }, { size: 0.5, orientation: 1, groups: [{ size: 0.6 }, { size: 0.4 }] }],
      });
    }
